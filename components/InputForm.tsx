  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 mb-8">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          <h2 className="text-xl font-bold text-gray-900">Nhập Token</h2>
          <p className="text-sm text-gray-500">Dán Authorization Token để tải dữ liệu</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="relative">
          <div className="flex justify-between items-center mb-2">
            <label htmlFor="token" className="text-sm font-semibold text-gray-700">Token (Bearer ...)</label>
            {!tokenText && (
              <button
                type="button"
                onClick={handlePaste}
          ...
          <textarea
            id="token"
            rows={3}
            className="block w-full rounded-xl border-gray-300 border focus:border-blue-500 focus:ring-blue-500 p-3 text-sm shadow-sm font-mono placeholder-gray-400 bg-gray-50 text-gray-900 focus:bg-white transition-colors resize-none"
        ...
        <button
          ...
          className="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-xl shadow-md text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
        >
          {isLoading ? (
            <>

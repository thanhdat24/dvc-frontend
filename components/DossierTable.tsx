...
const DossierTable: React.FC<DossierTableProps> = ({ title, data }) => {
  if (!data || data.length === 0) {
    return (
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        ...
        <p className="text-gray-500 italic text-sm">Không có hồ sơ nào.</p>
      </div>
    );
  }
  ...
  const getUrgencyLabel = (appointmentDateStr?: string) => {
    ...
    try {
      ...
      if (diffDays <= 1) { // <= 24h
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-red-100 text-red-800">🔥 Hỏa tốc</span>;
      } else if (diffDays <= 3) {
        return <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-bold bg-yellow-100 text-yellow-800">⚠️ Khẩn</span>;
      }
    } catch (e) {
      return null;
    }
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-8">
      <div className="bg-gray-50 px-6 py-4 border-b border-gray-200">
        <h3 className="text-lg font-bold text-gray-800 flex items-center">
          ...
          {title} 
          <span className="ml-2 text-sm font-normal text-gray-500">({data.length} hồ sơ)</span>
        </h3>
      </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Mã hồ sơ</th>
              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider max-w-xs">Nội dung</th>
              <th scope="col" className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Thực hiện</th>
            ...
            {data.map((item, index) => {
              ...
              return (
                <tr key={item.id || index} className="hover:bg-blue-50 transition-colors">
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-medium text-blue-600">{item.code}</td>
                  <td className="px-4 py-4 text-sm text-gray-700 max-w-xs truncate" title={item.applicant?.data?.noidungyeucaugiaiquyet}>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-center text-sm font-medium">
                    {link !== '#' ? (
                      <a 
                        ...
                        target="_blank" 
                        rel="noreferrer"
                        className="text-white bg-blue-600 hover:bg-blue-700 px-3 py-1.5 rounded-md text-xs transition-colors"
                      >
                        Mở Link
                      </a>
                    ) : (
